...
data_merged = pd.concat([acc_df.iloc[:,:3],gyr_df], axis=1) # 0 is row-wise and 1 is column-wise
...
data_resmapled = pd.concat([df.resample(rule="200ms").apply(sampling).dropna() for df in days])

data_resmapled["set"] = data_resmapled["set"].astype("int")
data_resmapled.info()
...
data_resmapled.to_pickle("../../data/interim/01_data_processed.pkl")